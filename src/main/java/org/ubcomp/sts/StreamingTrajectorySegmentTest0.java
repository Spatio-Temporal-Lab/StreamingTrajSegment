package org.ubcomp.sts;

import org.apache.flink.streaming.api.datastream.DataStreamSource;
import org.apache.flink.streaming.api.datastream.SingleOutputStreamOperator;
import org.apache.flink.streaming.api.environment.StreamExecutionEnvironment;
import org.ubcomp.sts.datasource.SourceInfinity;
import org.ubcomp.sts.object.GpsPoint;
import org.ubcomp.sts.operator.ProcessFunction;
import org.ubcomp.sts.partition.CustomPartitioner;


/**
 * Execute the flink program
 *
 * @author syy
 **/

public class StreamingTrajectorySegmentTest0 {
    public static void main(String[] args) throws Exception {

        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        DataStreamSource<GpsPoint> GPSStream = env.addSource(new SourceInfinity());
        SingleOutputStreamOperator<Object> keyedStream = GPSStream.keyBy(new CustomPartitioner<>())
                .process(new ProcessFunction(45, 1200000));
        env.execute("Infinity");

    }
}
